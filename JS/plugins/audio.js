...
//	console.log(webAudioContext);
	...
	function localAudio(opts){
		...
		this.audio.loop=opts.loop;//濡傛灉loop璁剧疆鎴恡rue灏辨棤娉曟纭幏寰梕nded浜嬩欢
		this.audio.load();
...
//  	console.log(get_src(this.src)+' pause');
	...
	function webAudio(opts){
		this.context=new (window.AudioContext || window.webkitAudioContext)(); /* 鍒涘缓涓€涓� AudioContext */
	...
	webAudio.prototype.load=function(){
		var _this=this;
	    var xhr = new XMLHttpRequest(); /*涓€涓柊鐨� XHR 瀵硅薄 */
	    xhr.open('GET', this.src, true); /* 閫氳繃 GET 璇疯繛鎺ュ埌 .mp3 */
	    xhr.responseType = 'arraybuffer'; /* 璁剧疆鍝嶅簲绫诲瀷涓哄瓧鑺傛祦 arraybuffer */
	    ...
	    function init(arrayBuffer){
			_this.context.decodeAudioData(arrayBuffer, function(buffer) {
				_this.buffer = buffer;  /* 灏� buffer 浼犲叆瑙ｇ爜 AudioBuffer. */
				if(_this.autoplay) _this.play();
		    }, function(e) {
	...
	//鍦ㄦ墜鏈烘祻瑙堝櫒閲岋紝webaudio鍙互璋冭妭闊抽噺,audio鐩墠杩樹笉琛�
	...
	webAudio.prototype.play=function(currentTime){
	    ...
	    _this.source.connect(_this.context.destination); /*杩炴帴 AudioBufferSourceNode 鍒� AudioContext */
...
//  	console.log(get_src(this.src)+' pause');
	...
	audio.bgm=function(options){
		var defaults = {src:'',btn:$('.bgmBtn'),playClassName:'bgmPlay',stopClassName:'bgmStop',webAudio:false,autoplay:true};
	...
	function localAudioBgm(opts){
		...
		this.audio.load();
		if(os.weibo && os.ios){//瑙ｅ喅ios鐗堝井鍗氫笅锛屾棤娉曡Е鍙慳udio鐨勫悇绉峀oad浜嬩欢
			setTimeout(function(){
				init();
	...
	webAudioBgm.prototype.load=function(){
		var _this=this;
	    var xhr = new XMLHttpRequest(); //閫氳繃XHR涓嬭浇闊抽鏂囦欢
	    xhr.open('GET', this.src, true);
	    xhr.responseType = 'arraybuffer';
	    xhr.onload = function(e) { //涓嬭浇瀹屾垚
